...
app10: Video 10: import 'connect-mongo' (after sessions, it requires sessions).  This import (that which is being exported) is a function in which 
we need to pass session as a parameter.  Then, in the session middleware, we need to add the MongoStore.  We do this by adding a key/property
that is a new MongoStore.  This store will also have its own set of options.  One important key is to specify the mongoose connection key so that
we don't open two connections but instead use the existing mongoose connection.  Another key we need to add to the session middleware is the
cookie key so we can configure how long the cookie (and therefore session) should exist.  We do this using the maxAge key (in the cookie key)
to 3 hours (in milliseconds). 
Secondly, we want to make our session available in the views so we can directly access it in handlebars.  We do this in the same way we created
the login variable, so res.locals.session will make a session variable.  We will pass the req.session object to the session variable. 
*/ 
...
var index = require('./routes/index15');
var userRoutes = require('./routes/user4');
...
// view engine setup
app.engine('.hbs', expressHbs({defaultLayout: 'layout3', extname: '.hbs'}));
